    <div class="egov-page-header">
        <h1 class="egov-page-title">${displayName} 상세</h1>
        <div class="egov-button-group">
            <a th:href="@{${urlPrefix}List.do(bbsId=${r"${result."}${bbsId.javaName}${r"}"})}"
               class="krds-btn secondary medium egov-btn">목록</a>
    ...
    <div class="egov-attachment-box">
        <div class="egov-definition-grid">
            ...
            <div class="egov-definition-value">
                <a th:each="file : ${r"${fileList}"}"
                   th:href="@{/cmm/fms/FileDown.do(atchFileId=${r"${file.atchFileId}"},fileSn=${r"${file.fileSn}"})}"
                   class="egov-file-detail-link" download>
                    <svg aria-hidden="true" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M12 3v12"></path>
                        <path d="m7 10 5 5 5-5"></path>
                        <path d="M5 21h14"></path>
                    </svg>
                    <span th:text="${r"${file.originalFileName}"}">첨부파일 다운로드</span>
                    <span class="egov-file-id"
                          th:text="${r"${file.fileSize != null ? file.fileSize + ' bytes' : ''}"}">0 bytes</span>
                </a>
                <span th:if="${r"${#lists.isEmpty(fileList)}"}"
