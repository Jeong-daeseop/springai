...
- [~] **R0-028 · P1** FORM/LIST/DETAIL/DASHBOARD Layout과 Desktop/Tablet/Mobile 변환·Component Swap 정책 Schema, Java/Plugin 적용을 완료했다. 실제 Figma MCP에서 Desktop `388:1060`은 정상 렌더링을 확인했지만 Mobile `576:1489`는 390px 외곽 안에 730px 6열 Table이 남아 **Table→Card 시각 검증에 실패**했다. 증적은 `docs/figma/evidence/2026-08-21-r0-runtime/desktop-mobile-visual-verification.md`에 기록했다.
- [~] **R0-029 · P1** KRDS 색·타이포그래피·간격·radius Token fixture와 Catalog 매핑을 완료했다. FTC 공식 라이브러리 Search Panel `75:98`에서 Variable 18개를 MCP로 조회해 `ftc-token-variable-evidence.json`에 보존했다. Design System Library Publish 및 운영 Profile Binding에 대한 조직 승인은 잔여
...
- [~] **R5-T03** 순수 Reconciliation 테스트에서 신규 논리 노드만 `ADD` 판정 검증 — 신규 logicalNodeId만 ADD이고 기존 노드는 REUSE인 회귀 테스트 추가. Figma Desktop 런타임 검증 필요
...
- [~] **R5-T08** 7가지 요청 fixture의 Preview diff와 실제 Reconciliation 결과가 일치한다. — 7종 요청을 순차 생성·Reconciliation하는 순수 회귀 테스트를 추가해 Preview ADD/REUSE/ARCHIVE 일치를 고정했다. Figma Desktop 동일 화면 교차 Apply와 Mobile Table→Card 시각 확인은 잔여
...
- [~] **R8-023 · P1** MERGE 충돌 및 USER_OVERRIDE 보존 실패율 측정 — `FigmaOperationsService.metrics()`의 충돌·`USER_OVERRIDE_PRESERVATION_FAILED` 집계와 서비스 회귀 테스트를 유지하고, 실제 Figma 런타임에서 의도적으로 보존 실패 Issue를 발생시키는 fixture 검증은 잔여
...
| 버전 | 일자 | 변경 내용 |
|---|---|---|
| 4.27 | 2026-08-21 | Figma 쓰기 연결로 격리 QA Fixture `668:755`를 실제 생성하고 ADD 대상 `668:919`(`qa/r5-t03/new-node`)를 추가했다. 실제 Instance `668:759`에서 관리값 `State=default`와 현재값 `State=focus`를 분리해 USER_OVERRIDE divergence를 재현했다. 다만 Figma host가 private `setPluginData`를 지원하지 않고 `setSharedPluginData`만 지원해 운영 Plugin의 전체 Apply callback·Issue 발행까지는 실행하지 못했으며, 결과를 [R5/R8 증적](./evidence/2026-08-21-r0-runtime/r5-r8-plugin-apply-evidence.json)으로 기록했다. |
| 4.26 | 2026-08-21 | Figma MCP 읽기 연결로 `eGovFrame` Desktop Frame `388:1060`과 Mobile Frame `576:1489`를 실제 조회·스크린샷 저장. Desktop은 정상 렌더링했으나 Mobile은 390px 외곽 안에 730px 6열 Table이 남아 Table→Card 시각 검증 FAIL로 확정([증적](./evidence/2026-08-21-r0-runtime/)). FTC Search Panel `75:98`의 라이브 Variable 18개를 조회해 Token 증적 JSON을 저장했으며 조직 Publish/Binding 승인은 사람 결정으로 유지. R5-T03/T08 실제 Plugin Apply와 R8-023 의도적 보존 실패 fixture는 Figma Desktop Plugin 조작이 필요해 잔여. |
| 4.25 | 2026-08-21 | R0-028/R5-T03/R5-T08/R8-023 자동 검증 보강. Plugin 순수 테스트에 신규 logicalNodeId ADD·7종 순차 Reconciliation 시나리오를 추가하고 62건 통과. R0-029 Token fixture에 source/revision/contentHash/syncedAt provenance와 canonical SHA-256 drift Gate를 추가했다. Figma Desktop 신규 화면·Table→Card 시각 확인, 동일 화면 교차 Apply, 의도적 USER_OVERRIDE 보존 실패 런타임 fixture는 Desktop 수동 검증 잔여로 유지한다. |
...
| 4.32 | 2026-08-21 | Mobile Table→Card Pattern의 런타임 논리 타입을 임시 `egov.dataCard`에서 Catalog 계약의 `krds.cardList`로 정합화. `krds.dataTable→krds.cardList:MOBILE` swap 증적을 기록하며 `cardList`는 `krds.container` + `krds.card` 조합 Pattern으로 유지한다. |
| 4.33 | 2026-08-21 | Mobile `krds.cardList` 변환에 `cardCount`, `patternComposition`, `patternRole` 런타임 증적을 추가하고 카드 행이 없을 때 `CARD_ROWS_NOT_FOUND` 경고를 기록하도록 보강. Plugin typecheck/build/npm test(62건) 통과. |
