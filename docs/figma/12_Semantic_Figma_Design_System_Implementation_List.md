...
- [~] **R5-043 · P1** 멀티 스크린 Operation을 전체 Preview 성공 후 일괄 Apply하며 중간 실패 시 부분 적용 방지 — (2026-08-17) Plugin `core.ts:planMultiScreenApply()`(전체 사전 검증, 순수함수)와 `code.ts:applyMultiScreenBundles()`(순차 Apply + 실패 시 이미 커밋된 화면 배치 단위 rollback) 신설, UI(`LOAD_MULTI_BUNDLE`/`APPLY_MULTI`) 연결. (2026-08-18) R6-036에서 서버 측 화면별 Bundle 일괄 생성(all-or-nothing)도 구현 완료 — Plugin Apply 단계와 서버 Bundle 생성 단계가 각자 원자성을 보장하지만, 서버가 만든 여러 Bundle을 Plugin이 한 번에 내려받아 그대로 `LOAD_MULTI_BUNDLE`에 넘기는 배선(REST 다운로드 엔드포인트 연결)은 아직 없어 `[~]` 유지 — (2026-08-18 재조사) DEC-10 파일 우선 경로(1차 채택 경로)에서는 `LOAD_MULTI_BUNDLE`이 이미 읽어들인 Bundle 내용을 직접 받으므로 여러 파일을 수동으로 다운로드·업로드하면 오늘도 정상 동작한다. 서버는 `GET /{operationId}/info`(전체 artifacts 목록)와 `GET /screens/{screenId}/download`(개별 Bundle 다운로드, R6-003)를 이미 REST로 제공하므로 Java 쪽에 빠진 엔드포인트는 없다. 남은 것은 Plugin(TypeScript, `figma-screen-spec-plugin/`)이 이 두 엔드포인트를 반복 호출해 자동으로 여러 Bundle을 받아오는 UX 편의 기능 하나뿐이며, 별도 언어/툴체인(esbuild/eslint/node --test)의 작업이라 이번 Java 구현 세션 범위 밖으로 유지
...
- [~] **R5-T08** 7가지 요청 fixture의 Preview diff와 실제 Reconciliation 결과가 일치한다. — (2026-08-18) TEXT_DESCRIPTION 외 REFERENCE_STYLE·IMAGE_REFERENCE·COMPONENT_SPECIFIED·MODIFY_EXISTING·MULTI_SCREEN_FLOW·PLATFORM_CONVERT(6종 전체)가 R6-033~038로 실제 Bundle 생성 경로를 갖춰 `FigmaDesignOrchestrationServiceTest`의 `generateBundleFor*` 테스트들이 각 유형의 Preview 결과(PREVIEW_READY 전이·ArtifactRef)를 검증한다(7종 전체 커버). **다만 이 fixture 테스트들은 각 요청 유형이 개별적으로 Bundle을 만드는 경로만 검증할 뿐, "동일 화면에 대해 7종 요청을 실제로 교차 적용했을 때 Preview diff와 Reconciliation이 정확히 일치하는가"라는 원래 항목명의 교차 시나리오까지 검증하지는 않아 `[~]`로 유지**
...
- [~] **R6-T04** 전체 Spring Context와 기존 테스트 회귀 통과. 실제 Streamable HTTP JSON-RPC E2E 호출은 후속 운영 검증 필요 — (2026-08-18 재조사) `McpToolDefinitionSnapshotTest`가 이미 Spring Context에서 `ToolCallbackProvider`/`MethodToolCallback` 등록을 exhaustively 검증하므로 Tool 계층 자체는 커버됨. **실제 Streamable HTTP wire 프로토콜(initialize handshake → `Mcp-Session-Id` 헤더 → `tools/call` → SSE/JSON 이중 응답 모드) E2E는 여전히 없음** — 손수 만든 JUnit HTTP client로 이 세션 프로토콜을 흉내 내면 우리 코드가 아니라 Spring AI MCP 프레임워크 자체의 전송 구현을 재검증하게 되어(버전 업그레이드마다 우리 코드와 무관하게 깨질 위험) 가치 대비 리스크가 크다고 판단, 손으로 작성하지 않음. 실제 MCP 클라이언트 라이브러리를 쓰는 운영 smoke test로 별도 진행 권장
...
**2-A5: 요청 흐름 연결 (3시간)**
- [~] **R6-030 · P0** `FigmaDesignRequestRouter`의 명시/컨텍스트 판정은 구현. 7개 전용 Tool이 명시 타입을 조립하므로 저장소에는 유효 타입만 들어가지만 자유 텍스트 router와 구조화 분류의 단일 경로 통합은 미완료 — (2026-08-18 재조사) `FigmaDesignRequestRouter`는 프로덕션 호출자도 테스트도 전혀 없는 완전한 죽은 코드임을 확인(자체 4-value `DesignRequestType` enum을 갖는데, 실제 시스템은 R6-032~038에서 확정된 7-value `FigmaDesignRequestType`을 쓴다 — 서로 다른 세대의 설계). "자유 텍스트 router와 구조화 분류의 단일 경로 통합"이라는 원래 목표는 이 Router가 아니라 **7개 전용 MCP Tool이 각자 명시적 타입으로 `FigmaDesignRequest`를 조립하는 현재 아키텍처로 이미 달성됨**(저장소에는 항상 유효 타입만 들어감, R6-T12로 검증). 남은 것은 이 orphan 클래스를 삭제하거나 재활용할지에 대한 결정뿐이며 이번 세션 범위 밖(코드 삭제는 별도 승인 필요)
- [x] **R6-031 · P0** `FigmaDesignOrchestrationService` 구현 — 7개 요청은 canonical hash 기반 영속 `ANALYZED` 승인 후보로 저장된다. (2026-08-18) `generateBundle()` 신설로 REFERENCE_STYLE·IMAGE_REFERENCE·COMPONENT_SPECIFIED·MODIFY_EXISTING·MULTI_SCREEN_FLOW·PLATFORM_CONVERT(승인 Spec 경로의 TEXT_DESCRIPTION 포함 7/7종 전체)가 ANALYZED 이후 실제 ScreenSpecification(또는 기존 명세)→FigmaScreenSpec→Bundle→불변 Artifact→영속 `PREVIEW_READY`로 연결된다. 자연어 prompt를 DB 테이블 선택이나 필드 단위 diff로 자동 해석하는 기능은 여전히 없다(DB 스키마가 항상 "무엇을 생성할지" 결정)
- [~] **R6-032 · P0** `create_design_from_text(prompt, platform)` MCP callback 구현 — DB Schema→ScreenSpecification→FigmaScreenSpec 기반 흐름은 존재하며(승인 Spec 경로), 자연어 prompt를 DB 테이블 선택으로 바꾸는 구조화 분석은 여전히 없음(변경 없음, 이번 범위 밖)
