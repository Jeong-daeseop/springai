...
- [~] `.figpack` 캡처를 후보 Screen Specification으로 변환하고 승인 후 의미 흐름을 탈 수 있다. 백엔드 `.figpack` 변환·품질 평가·후보 승인·Semantic Bundle 경로와 Figma Desktop 7화면 Apply는 완료했고, 인증 로그인 fixture E2E와 0.1% visual tolerance 정책도 검증했다. 남은 범위는 실제 운영 로그인 URL 캡처와 원본/생성 화면의 픽셀 비교(R7-002/R7-015/R7-T04)
