...
> ⚠️ **대체됨 (2026-08-17)**: 이 체크리스트는 Registry `2.1.0`·Q&A 6화면 기준으로 작성됐으나,
> 이후 Registry `2.2.2`·Q&A 7화면(`qna-update` 추가) 기준으로 운영 전환 승인·MERGE가 완료됐다.
> 최신 승인 결과는 `KRDS_QNA_7화면_운영검증보고서_2026-08-16.md`를,
> 최신 백로그·진행 상태는 `KRDS_Figma_Role_Variant_구현목록.md`(KRV-070 등)를 참고한다. 이 문서는
> 6화면/Registry 2.1.0 시점의 승인 절차 기록으로만 보존한다.
...
## 5. 운영 전환 판정 (2026-08 당시 기준)

당시에는 **전환 불가**였다. A~D의 미완료 항목과 네 역할의 승인 기록이 모두 채워진 뒤에만
`PUBLISHED` 전환이 가능했다. 이후 Registry `2.2.2`·Q&A 7화면 기준으로 별도 승인 절차를 거쳐
전환이 완료됐다(문서 상단 대체 안내 참고).
