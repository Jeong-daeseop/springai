...
> 2026-08-27, 코드 실측 기준 작성. 배경은 [`화면생성Tool_3종_비교분석.md`](./화면생성Tool_3종_비교분석.md)
> 및 아키텍처 다이어그램 아티팩트 5번 섹션(DesignReferenceTool)에서 확인한 "색상·토큰이 두 단계에
> 걸쳐 사라진다"는 발견의 후속 검토다.
>
> **상태(2026-09-02 갱신): 구현 완료.** 이 검토를 바탕으로 작성된
> [`Figma_fills_strokes_구현계획.md`](./Figma_fills_strokes_구현계획.md)가 커밋 `bbc8079`(feat: capture
> Figma component fills/strokes for claude-path prompts)로 반영됐다. 이 문서는 검토 당시 근거를
> 보존하는 기록으로 유지한다.
